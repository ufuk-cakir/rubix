...
def handler_with_mock_data(mock_simulation, mock_config):
    ...
    handler = PynbodyHandler(
        path="mock_path", halo_path="mock_halo_path", config=mock_config, dist_z=dist_z
    )
...
def test_get_galaxy_data(handler_with_mock_data):
    ...
    expected_halfmassrad_stars = handler_with_mock_data.calculate_halfmass_radius(
        positions, masses
    )

    assert (
        galaxy_data["redshift"] == expected_redshift
    ), f"Expected redshift {expected_redshift}, got {galaxy_data['redshift']}"
    assert np.allclose(
        galaxy_data["center"], expected_center
    ), f"Expected center {expected_center}, got {galaxy_data['center']}"
    assert (
        "halfmassrad_stars" in galaxy_data
    ), "halfmassrad_stars is missing from galaxy_data."
    assert np.isclose(
        galaxy_data["halfmassrad_stars"], expected_halfmassrad_stars
