...
* Python source files:
  * The Python package source files are located in the `rubix` directory.
  * `tests/test_rubix.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `rubix`.
