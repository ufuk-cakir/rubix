...
* Python source files:
  * The Python package source files are located in the `virtual_telescope` directory.
  * `tests/test_virtual_telescope.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `virtual_telescope`.
